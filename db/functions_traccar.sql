...
BEGIN
  RETURN QUERY
    SELECT MIN(time_stamp) AS min_date, MAX(time_stamp) AS max_date
      FROM traccar_track
INNER JOIN traccar_trips USING (trip_id)
     WHERE in_vessels IS NULL
        OR device_id = ANY(in_vessels);
END;
$FUNC$ LANGUAGE plpgsql SECURITY DEFINER STABLE;
--}}}
...
BEGIN
  RETURN QUERY
    SELECT t.trip_id, d.device_name, 
           (TO_CHAR(MIN(t.time_stamp), 'YYYY-MM-DD HH24:MI') || ' - ' || TO_CHAR(MAX(t.time_stamp), 'YYYY-MM-DD HH24:MI'))::VARCHAR(255) AS trip_label
      FROM traccar_track AS t
INNER JOIN traccar_trips USING (trip_id)
INNER JOIN traccar_devices AS d USING (device_id)
     WHERE ((min_date IS NULL OR max_date IS NULL) OR DATE_TRUNC('day', t.time_stamp) BETWEEN min_date AND max_date)
